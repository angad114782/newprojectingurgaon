...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const pageUrl = `${siteUrl}/3-bhk-flats-bhiwadi`;
  return {
    title: `3 BHK Flats in Bhiwadi 2025 | from ₹55 Lakh | Best Options | ${settings.siteName}`,
    description: `Compare 3 BHK flats in Bhiwadi from ₹55 Lakh. Omaxe (₹65L), Ashiana (₹62L), Avalon Sports City (₹75L), BPTP Neemrana (₹70L) and more. RERA verified. Free site visit. Zero brokerage. RERA verified. Call ${settings.phone}.`,
    keywords: '3 bhk flats bhiwadi, 3 bhk bhiwadi price, bhiwadi 3 bhk apartments',
    openGraph: { title: `3 BHK Flats in Bhiwadi 2025 | from ₹55 Lakh | Best Options | ${settings.siteName}`, url: pageUrl, type: 'website' },
    alternates: { canonical: pageUrl },
  };
}

export default async function Page() {
  const projects = await fetchApiProjects({ config: '3 BHK', limit: 50 });
  return (
    <>
      <nav className="bg-brand-mint/30 border-b border-brand-border/40 py-3">
        <div className="max-w-7xl mx-auto px-4 text-sm text-brand-muted">
          ...
          <span className="text-brand-dark font-medium">3 BHK Flats in Bhiwadi</span>
        </div>
      </nav>
      <section className="hero-gradient py-16">
        <div className="max-w-7xl mx-auto px-4">
          <div className="max-w-3xl">
            <span className="inline-block bg-brand-accent/20 text-brand-accent text-sm font-semibold px-4 py-1.5 rounded-full mb-4">🏠 3 BHK</span>
            <h1 className="text-4xl md:text-5xl font-display font-bold text-white leading-tight mb-4">3 BHK Flats in Bhiwadi</h1>
            <p className="text-white/80 text-lg leading-relaxed mb-8">Compare 3 BHK flats in Bhiwadi from ₹55 Lakh. Omaxe (₹65L), Ashiana (₹62L), Avalon Sports City (₹75L), BPTP Neemrana (₹70L) and more. RERA verified.</p>
      ...
      <section className="py-14 bg-brand-mint/20" id="projects">
        <div className="max-w-7xl mx-auto px-4">
          <h2 className="text-2xl font-display font-bold text-brand-text mb-2 text-center">3 BHK Flats in Bhiwadi</h2>
      ...
      <section className="py-16 bg-brand-dark" id="lead-form">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <h2 className="text-3xl font-display font-bold text-white mb-3">Get Property Details — 3 BHK Flats in Bhiwadi</h2>
