...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const pageUrl = `${siteUrl}/plots-in-bhiwadi`;
  return {
    title: `Plots in Bhiwadi 2025 | Residential & RIICO Industrial Plots | ${settings.siteName}`,
    description: `Residential and RIICO industrial plots in Bhiwadi. From 100 sqyd residential plots to 200 sqm RIICO industrial plots. Government-backed RIICO allocation available. Free site visit. Zero brokerage. RERA verified. Call ${settings.phone}.`,
    keywords: 'plots in bhiwadi, bhiwadi plots, residential plots bhiwadi, riico plots bhiwadi',
    openGraph: { title: `Plots in Bhiwadi 2025 | Residential & RIICO Industrial Plots | ${settings.siteName}`, url: pageUrl, type: 'website' },
    alternates: { canonical: pageUrl },
  };
}

export default async function Page() {
  const projects = await fetchApiProjects({ config: 'Plot', limit: 20 });
  return (
    <>
      <nav className="bg-brand-mint/30 border-b border-brand-border/40 py-3">
        <div className="max-w-7xl mx-auto px-4 text-sm text-brand-muted">
          ...
          <span className="text-brand-dark font-medium">Plots in Bhiwadi</span>
        </div>
      </nav>
      <section className="hero-gradient py-16">
        <div className="max-w-7xl mx-auto px-4">
          <div className="max-w-3xl">
            <span className="inline-block bg-brand-accent/20 text-brand-accent text-sm font-semibold px-4 py-1.5 rounded-full mb-4">🏞️ Plot Investment</span>
            <h1 className="text-4xl md:text-5xl font-display font-bold text-white leading-tight mb-4">Plots in Bhiwadi</h1>
            <p className="text-white/80 text-lg leading-relaxed mb-8">Residential and RIICO industrial plots in Bhiwadi. From 100 sqyd residential plots to 200 sqm RIICO industrial plots. Government-backed RIICO allocation available.</p>
      ...
      <section className="py-14 bg-brand-mint/20" id="projects">
        <div className="max-w-7xl mx-auto px-4">
          <h2 className="text-2xl font-display font-bold text-brand-text mb-2 text-center">Plots in Bhiwadi</h2>
      ...
      <section className="py-16 bg-brand-dark" id="lead-form">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <h2 className="text-3xl font-display font-bold text-white mb-3">Get Property Details — Plots in Bhiwadi</h2>
